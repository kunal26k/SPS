...
Every page adapts to three sizes: phone (≤640px), tablet (641–1024px) and desktop.

- Website pages: each `src/site/*/page.css` and `src/app/(site)/about/about.css` ends with a tablet and a phone layer (two- and three-column grids on tablets, stacked on phones). The shared nav switches to a menu below 980px.
- Portal screens: `src/design/responsive/*.css` is added after each screen's own CSS by `pnpm import:design` — `portal.css` for every portal, plus `calendar.css`, `messages.css`, `admin-portal.css` and `feature-documentation.css`.
